    ...
    <q-input filled v-model="value" @update:model-value="$emit('update:text', value)"
      ...
      <template v-if="props.isDate" v-slot:prepend>
        <q-icon name="event" class="cursor-pointer">
          <q-popup-proxy cover transition-show="scale" transition-hide="scale">
            <q-date v-model="value" :mask="props.format" @update:model-value="$emit('update:text', value)">
      ...
      <template v-slot:append>
        <q-icon name="access_time" class="cursor-pointer">
          <q-popup-proxy cover transition-show="scale" transition-hide="scale">
            <q-time v-model="value" :mask="props.format" format24h @update:model-value="$emit('update:text', value)">
              <div class="row items-center justify-end">
                <q-btn v-close-popup label="Close" color="primary" flat />
              </div>
            </q-time>
