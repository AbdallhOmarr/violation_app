    ...
    <q-dialog :dir="$i18n.locale == 'en' ? ' ltr' : 'rtl'" :model-value="props.modelValue" full-width>
        <form-component :titel="props.edit ? 'تعديل بيانات المخالفة ' : 'اضافة مخالفة'" @clear:data="clear"
            ...
            <template #content>
                ...
                <InputText label="ملاحظات المخالفة " col="12" @update:text="setDetails"
                    :default="request?.violation_details" type="textarea">
                </InputText>
            </template>
        </form-component>
    </q-dialog>
...
const setType = (val) => {
    console.log(val)
    request.value.degree = val?.degree || type.filter((el) => el.id == val)?.[0].degree
...
const save = async () => {
    ...
    props.edit ? await violationStore.editViolations(request.value.id) : await violationStore.addViolations(request.value.trainee)
