<template>
  <HeaderCard :titel="props.titel" :icon="props.icon">
    <template #btn>
      <slot name="btn"> </slot>
    </template>
    <template #content>
        ...
        <q-form @submit.prevent.stop="$emit('update:data')" @reset="$emit('clear:data')" class="row q-mx-sm">
          ...
          <div class="row">
            ...
            <div>
              ...
              <q-btn v-if="props.showCanselButton" label="الغاء" type="reset" color="secondary" flat
                class="q-mx-sm q-my-sm" v-close-popup />
            </div>
          </div>
        </q-form>
      </div>
