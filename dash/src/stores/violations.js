...
export const useViolationStore = defineStore('violation', () => {
    ...
    const show = async (id) => {
        try {
            ...
            loading.value = { type: 'details', value: false };
        }
    };

    const addViolations = async (traineeId) => {
        try {
            loading.value = { type: 'change', value: true };
            await api.post('violations/', requestInfo.value);

            await index()
            list.value = list.value.filter((el) => el.trainee == traineeId)
        } catch (err) {
        } finally {
            loading.value = { type: 'change', value: false };
        }
    ...
    const editViolations = async (id) => {
        try {
            ...
            await index();
            list.value = list.value.filter((el) => el.trainee == request.value.trainee)
        } catch (err) {
        } finally {
            loading.value = { type: 'change', value: false };
        }
    ...
    const destroy = async (id, traineeId) => {
        try {
            loading.value = { type: 'change', value: true };
