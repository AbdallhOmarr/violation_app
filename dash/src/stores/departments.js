...
export const useDepartmentsStore = defineStore('departments', () => {
    ...
    const request = ref({ department_name: undefined, department_major: undefined });

    const resetRequest = () => {
        request.value = { department_name: undefined, department_major: undefined };
    };

    const requestInfo = computed(() => {
        return { department_name: request.value.department_name, department_major: request.value.department_major };
    });
    ...
    const editDepartments = async (id) => {
        try {
            loading.value = { type: 'change', value: true };
            await api.put(`departments/${id}/`, requestInfo.value);
            index()
        } catch (err) {
        } finally {
            loading.value = { type: 'change', value: false };
    ...
    return {
        loading, list, request, index, addDepartments, destroy, resetRequest, editDepartments
    };
});
