...
export const useTrainerStore = defineStore('trainer', () => {
    ...
    const show = async (id) => {
        try {
            loading.value = { type: 'detsils', value: true };

            const res =
                await api.get(`trainers/${id}/`)
            details.value = res.data;
        } catch (err) {
        } finally {
            loading.value = { type: 'detsils', value: false };
        }
    };
    ...
    const editSetting = async (id, data) => {
        try {
            loading.value = { type: 'change', value: true };
            await api.put(`trainers/${id}/`, data);
    ...
    return {
        editSetting,
        loading, list, details, request, index, addTrainers, editTrainers, destroy, resetRequest, show
    };
});
